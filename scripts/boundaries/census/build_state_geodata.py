#!/usr/bin/env python3
"""
Export US state boundaries as Foundry-ready GeoJSON and CSV.

Source:
  - TIGER/Line 2024 state shapefile (national)

Outputs:
  - data/boundaries/census/us_states.geojson
  - data/boundaries/census/us_states.csv
...
STATE_SHP = os.path.join(SOURCE_DIR, "tl_2024_us_state", "tl_2024_us_state.shp")

OUT_GEOJSON = os.path.join(BOUNDARY_DIR, "us_states.geojson")
OUT_CSV = os.path.join(BOUNDARY_DIR, "us_states.csv")

TARGET_CRS = "EPSG:4326"

GEOJSON_COLUMNS = [
    "STATEFP", "GEOID", "STUSPS", "NAME", "LSAD",
    "ALAND", "AWATER", "INTPTLAT", "INTPTLON", "geometry",
]

CSV_COLUMNS = [
    "state_fips", "geoid", "stusps", "name", "lsad",
...
def build_geojson():
    ...
    gdf = gdf.sort_values("STATEFP").reset_index(drop=True)
    gdf.to_file(OUT_GEOJSON, driver="GeoJSON")
    print(f"  {len(gdf)} states/territories -> {OUT_GEOJSON}")
    return gdf
...
def build_csv():
    ...
    prop_keys = ["STATEFP", "GEOID", "STUSPS", "NAME", "LSAD",
...
def main():
    if not os.path.exists(STATE_SHP):
        print(f"Error: {STATE_SHP} not found", file=sys.stderr)
        print("Download from: https://www2.census.gov/geo/tiger/TIGER2024/STATE/tl_2024_us_state.zip", file=sys.stderr)
        sys.exit(1)
